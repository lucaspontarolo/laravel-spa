...
export default {
  ...
  methods: {
    handleUpdate() {
      ...
      this.isLoading = true;
      this.axios
        .put(`/api/cars/${this.car.id}`, payload)
        .then((response) => {
    ...
    fetchCurrentCar() {
      ...
      this.axios
        .get(`/api/cars/${id}`)
        .then((response) => {
          Vue.set(this, 'car', response.data);
        })
        .catch((err) => {
          this.$toast({ icon: 'error', title: 'Ocorreu um erro ao carregar as informações do carro.' });
